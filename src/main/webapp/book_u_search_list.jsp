...
<head>
	...
	.sectiondiv{ height:auto; min-height: 100%; padding-bottom: 180px; background-color: #f3f3f3; }
...
<div class="carConSize">
	...
	<div class="container" >
		...
		<div id="btn-div" style="display : flex;">
			<button type="button" class="btn btbtn btbtnl" onclick="location.href='./bookUListAction.book'">전체 도서 목록</button>
			<button type="button" class="btn btbtn" onclick="location.href='main.jsp'">메인 페이지</button>
		</div>
		...
		<% if(nowPage == 0){%>
			<div id="pageList">
				<%if(nowPage<=1){ %> 
					[이전]&nbsp;
				<%}else{ %>
					<a href="./bookUListAction.book?page=<%=nowPage-1 %>">[이전]</a>&nbsp;
				<%} %>
				...
				<%for(int a=startPage;a<=endPage;a++){
						if(a==nowPage){%>  
							[<%=a %>]
						<%}else{ %>
							<a href="./bookUListAction.book?page=<%=a %>">[<%=a %>]</a>&nbsp;
						<%} %>
				<%} %>
				...
				<%if(nowPage>=maxPage){ %>
					[다음]
				<%}else{ %>
					<a href="bookUListAction.book?page=<%=nowPage+1 %>">[다음]</a>
				<%} %>
			</div>
		<%} %>
	</div>
